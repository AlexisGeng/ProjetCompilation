...
public class Generateur {
	...
	private Object genererInstruction(Noeud nf) {
		...
		case Noeud.DEC:
			for (Noeud n : nf.getFils()) {
				...
				Symbole s = tds.get("null _ " + t[0]); // on r√©cup√®re l'√©l√©ment avant la variable
	...
	/**
	 * MÈthode qui gÈnËre les diffÈrentes expressions possibles du programme
	 * @param nf: noeud
	...
	/**
	 * MÈthode pour gÈnÈrer les fonctions de lecture
	 * @return StringBuffer: code assembleur
	 */
	private StringBuffer genererLecture() {
		StringBuffer res = new StringBuffer("|GÈnÈrer lecture\n");
		res.append("\n.....\n\tPUSH()"); 
		return res;
	}

	/**
	 * MÈthode pour gÈnrer les fonctions d'Ècritue
	 * @param nf:noeud
	 * @return StringBuffer: code assemebleur
	 */
	private StringBuffer genererEcriture(Noeud nf) {
		StringBuffer res = new StringBuffer("|GÈnÈrer ecriture\n");
	...
	private StringBuffer genererBoucle(Noeud nf) {
		StringBuffer res = new StringBuffer("|GÈnÈrer Boucle\n");
		res.append("\nWhile"+nbrBoucle+":\n\t");
		res.append("\n\t"+this.genererExpression(nf.getFils().get(0))); //pas s√ªre que ca soit bien expression et non condition
		res.append("\n\tPOP(r0)\n\tBF(Done"+nbrBoucle+":)");
	...
	private StringBuffer genererReturn(Noeud nf) {
		StringBuffer res = new StringBuffer("|GÈnÈrer Return\n");
	...
	private StringBuffer genererIf(Noeud nf) {
		StringBuffer res = new StringBuffer("|GÈnÈrer if\n");
		res.append("\nsi" + nbrSi + ":\n\t");
		res.append(genererExpression(nf.getFils().get(0)) + "\n\tPOP(r0)\n\tBF(r0,sinon" + nbrSi + ")");
	...
	/**
	 * MÈthode qui permet de gÈnÈrer les bloc
	 * @param nf: noeud
	 * @return StringBuffer: code assembleur
	 */
	private StringBuffer genererBloc(Noeud nf) {
		StringBuffer res = new StringBuffer("|GÈnÈrer bloc\n");
	...
	/**
	 * MÈthode qui va gÈnÈrer les affectations
	 * @param nf: noeud
	 * @return StringBuffer: code assembleur
	 */
	private StringBuffer genererAffectation(Noeud nf) {
		StringBuffer res = new StringBuffer("|GÈnÈrer affectation\n");
		Noeud filsgauche = nf.getFils().get(0);
		Noeud filsdroit = nf.getFils().get(1);
	...
	/**
	 * MÈthode qui permet l'Ècriture du StringBuffer
	 * @param sb : String qui va Ítre Ècrit en assembleur
	 */
